...
def upload_to_ftp(ftp: FTP_TLS, file_source: Path):                         # uploads a file to the specified directory on the FTP server
    with open(file_source, "rb") as fp:                                     # with statement to open the local file specifed by file_source in read-binary mode ('rb')
        ...
        ftp.cwd("/home/ftpuser/ftp")                                        # specified the destination directory on the FTP server where the file will be uploaded
        destination_path = f"{file_source.name}"                            # full destination path on the FTP server by combining the specified destination directory and file name from the local file
